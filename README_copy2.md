...
| Step | 내용 |
|------|------|
| 1 | 데이터 수집 + 피처 엔지니어링 (`s1_data.py`) |
| 2 | Optuna 하이퍼파라미터 튜닝 — RMSE 최소화 (`s2_tune.py`) |
| 3 | 피처 선택 — 다중공선성 제거 + RFE (`s3_select.py`) |
| 4 | Bear 최적화 — AsymLoss 최소화 (`s4_optimize.py`) |
| 5 | 최종 평가 + 시각화 (`s5_evaluate.py`) |
...
| Step | 내용 |
|------|------|
| 1 | 분기 날짜 생성 + 타겟 수익률 산출 (`s1_dates.py`) |
| 2 | A~E 그룹 피처 데이터 수집 (`s2_data.py`) |
| 3 | Stage 1 Expanding Window 예측 (`s3_stage1_feat.py`) |
| 4 | 피처 엔지니어링 + 피처 선택 (`s4_features.py`) |
| 5 | XGBoost Optuna 튜닝 + Dynamic Sample Weight (`s5_tune.py`) |
| 6 | 최종 평가 + 시각화 (`s6_evaluate.py`) |
